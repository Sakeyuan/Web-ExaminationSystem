<template>
    <div v-if="tableData && tableData.length> 0">
        ...
        <el-table :data="tableData" :header-cell-class-name="tableTitle">
            ...
            <el-table-column prop="operate" label="操作" align="center">
                <template slot-scope="scope">
                    <el-button type="primary" @click="correctPaper(scope.row.paperId,scope.row.studentId)">
                        批改<i class="el-icon-edit" style="margin-left: 2px"></i>
                    </el-button>
                </template>
            </el-table-column>
        </el-table>
    ...
    <el-empty v-else description="(＾－＾) 没有试卷批改"></el-empty>
</template>
<script>
        ...
        methods: {
            ...
            load() {
                try {
                    ...
                    const res = this.$api.paperObj.getNoCorrectPaper(data).then(res => {
                        if (res.code === 2000) {
            ...
            correctPaper(paperId, studentId) {
                console.log("paperId: " + paperId);
                console.log("studentId: " + studentId);
                this.$router.push({
                    path: "/teacher/correctPaper",
                    query: {
                        paperId: paperId,
                        studentId: studentId
                    }
